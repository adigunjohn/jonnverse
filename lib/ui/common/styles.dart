import 'package:flutter/material.dart';
import 'package:jonnverse/ui/common/strings.dart';

//Colors
const Color kCBlueShadeColor = Color(0xFF3B5BDB);
const Color kCDarkAppbarColor = Color(0xFF121212);
const Color kCAccentColor = Color(0xFF00F2C3);
const Color kCOnAccentColor = Color(0xFF1B1B1B);
const Color kCBlackColor = Colors.black;
const Color kCWhiteColor = Colors.white;
const Color kCRedColor = Colors.red;
const Color kCGreenColor = Colors.green;
const Color kCTransparentColor = Colors.transparent;
const Color kCGreyColor = Colors.grey;
Color kCGrey100Color = Colors.grey.shade100;
Color kCGrey200Color = Colors.grey.shade200;
Color kCGrey300Color = Colors.grey.shade300;
Color kCGrey700Color = Colors.grey.shade700;
Color kCGrey800Color = Colors.grey.shade800;
Color kCGrey900Color = Colors.grey.shade900;
const Color kCBlack87Color = Colors.black87;

//TextStyles
const TextStyle kTSplashText = TextStyle(
  fontFamily: AppStrings.poppins,
  fontWeight: FontWeight.bold,
  fontSize: 27,
  color: kCWhiteColor,
  overflow: TextOverflow.ellipsis,
);
const TextStyle kTNavBarText = TextStyle(
  fontFamily: AppStrings.poppins,
  fontWeight: FontWeight.bold,
  color: kCWhiteColor,
  fontSize: 14,
  overflow: TextOverflow.ellipsis,
);
const TextStyle kTButtonText = TextStyle(
  fontFamily: AppStrings.poppins,
  fontWeight: FontWeight.bold,
  color: kCWhiteColor,
  fontSize: 15,
  overflow: TextOverflow.ellipsis,
);
const TextStyle kTAppBarText = TextStyle(
  fontFamily: AppStrings.poppins,
  fontWeight: FontWeight.bold,
  color: kCWhiteColor,
  fontSize: 24,
  overflow: TextOverflow.ellipsis,
);
const TextStyle kTTextSize21 = TextStyle(
  fontFamily: AppStrings.poppins,
  fontWeight: FontWeight.normal,
  fontSize: 21,
  overflow: TextOverflow.ellipsis,
);
const TextStyle kTTextSize18 = TextStyle(
  fontFamily: AppStrings.poppins,
  fontWeight: FontWeight.normal,
  fontSize: 18,
  overflow: TextOverflow.ellipsis,
);
const TextStyle kTTextSize16 = TextStyle(
  fontFamily: AppStrings.poppins,
  fontWeight: FontWeight.normal,
  fontSize: 16,
  overflow: TextOverflow.ellipsis,
);
const TextStyle kTTextSize15 = TextStyle(
  fontFamily: AppStrings.poppins,
  fontWeight: FontWeight.normal,
  fontSize: 15,
  overflow: TextOverflow.ellipsis,
);
const TextStyle kTTextSize12 = TextStyle(
  fontFamily: AppStrings.poppins,
  fontWeight: FontWeight.normal,
  fontSize: 12,
  overflow: TextOverflow.ellipsis,
);
const TextStyle kTTextSize12Grey = TextStyle(
  fontFamily: AppStrings.poppins,
  fontWeight: FontWeight.normal,
  fontSize: 12,
  color: kCGreyColor,
  overflow: TextOverflow.ellipsis,
);
const TextStyle kTTextSize16Grey = TextStyle(
  fontFamily: AppStrings.poppins,
  fontWeight: FontWeight.normal,
  fontSize: 16,
  color: kCGreyColor,
  overflow: TextOverflow.ellipsis,
);